...
def get_filters():
    ...
    while True:
        print('Which cities should we look for?')
        city = input(
            'Chicago(CH), New York City(NYC), or Washington(WA)? ').lower()
        print()
        if city == 'ch':
            city = 'chicago'
        ...
        if city not in CITY_DATA:
            print('Kindly enter a valid city: CH, NYC, or WA')
            continue
        city = CITY_DATA[city]
        break
    ...
    while True:
        choice = input(
            'Do you want to filter the data by month or week? (y)es/(n)o '
        ).lower()
        print()
        ...
        if choice:
            ...
            if filter == 'month':
                ...
                if month not in MONTH_DATA:
                    print('Sorry I did not understand that input. Try again.')
                    continue
                month = MONTH_DATA[month]
                day = 'all'
            elif filter == 'day':
                print('Which day\'s data to look at? ')
                day = input('Mon, Tues, Wed, Thur, Fri, Sat, Sun: ').lower()
                print()
...
def station_stats(df):
    ...
    print()
    print('Most commonly used start station as per our data was {}'.format(
        df['Start Station'].mode()[0]))

    # TO DO: display most commonly used end station
    print()
    print('Most commonly used end station as per our data was {}'.format(
    ...
    print('The most frequnt combination of start/end station was {}'.format(
...
def trip_duration_stats(df):
    ...
    print(
        'Passengers travelled a total of {} days, {} hours, {} minutes and {} seconds'.format(
            sum_days,
            sum_hours,
            sum_minutes,
            sum_seconds))
    ...
    print(
        'Passengers travelled an average of {} hours, {} minutes and {} seconds'.format(
            mean_hours,
            mean_minutes,
            mean_seconds))
...
def user_stats(df):
    ...
    for i in range(len(types_of_users)):
        print('{}s - {}'.format(
            types_of_users['User Type'][i], types_of_users['Start Time'][i]))
...
def display_data(df):
    choice = input(
        'Would you like to read some of the raw data? (y)es/(n)o: ').lower()
    print()
    if choice == 'yes' or choice == 'y':
        choice = True
    elif choice == 'no' or choice == 'n':
    ...
    if choice:
        while True:
            for i in range(5):
                print(df.iloc[i])
                print()
            choice = input('Another five? (y)es/(n)o: ').lower()
            if choice == 'yes' or choice == 'y':
                continue
            elif choice == 'no' or choice == 'n':
                break
...
def main():
    while True:
        ...
        restart = input(
            '\nWould you like to restart? Enter (y)es or (n)o: ').lower()
        print()
